...
import { projectExamples } from '@/app/session/examples';
import type {
...
describe('App project launcher and lifecycle', () => {
  ...
  it('authors and inspects independent keyframe tracks at the playhead', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'brightness keyframe at 0 beats' }),
    );
    await user.click(screen.getByRole('radio', { name: 'Ease in' }));
    expect(screen.getByRole('radio', { name: 'Ease in' })).toBeChecked();
    ...
    expect(
      screen.getByText('Add a later keyframe to shape this transition.'),
    ).toBeVisible();
    expect(screen.getByRole('radio', { name: 'Linear' })).toBeDisabled();
    ...
    expect(
      screen.getByText(
        '2 of 2 selected keys have an editable outgoing transition. Terminal keys and stepped colour keys are skipped.',
      ),
    ).toBeVisible();
    await user.click(screen.getByRole('radio', { name: 'In / out' }));
    expect(screen.getByRole('radio', { name: 'In / out' })).toBeChecked();
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'colour keyframe at 1 beats' }),
    );
    expect(
      screen.getByText(
        'Step colour interpolation ignores easing. Switch the layer to Smooth RGB to use it.',
      ),
    ).toBeVisible();
    expect(screen.getByRole('radio', { name: 'Linear' })).toBeDisabled();
  });
