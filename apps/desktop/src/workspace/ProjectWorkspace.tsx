...
import { useSongWorkspaceController } from '@/workspace/model/useSongWorkspaceController';
...
export function ProjectWorkspace({
  ...
  const songWorkspace = useSongWorkspaceController({
    activeSceneId,
    onExecuteCommand,
    project,
    setActiveSceneId,
    ...
    <main className="workspace-shell" style={workspaceStyle}>
      <WorkspaceToolbar
        ...
        onTimingCommit={(changes) => {
          if (songWorkspace.previewContext === 'song' && activeSong) {
            onExecuteCommand({
              changes: {
      ...
      <div className="workspace-editor">
        <AssetsPanel
          ...
          activeSongId={
            songWorkspace.previewContext === 'song'
              ? songWorkspace.activeSongId
              : null
          }
          ...
          onAddSong={songWorkspace.addSong}
          onSelectPalette={(id) => {
            setInspectorTarget({ id, kind: 'palette' });
            setFocusTokenId(null);
          }}
          onSelectGroup={selectProjectGroup}
          onSelectScene={activateStandaloneScene}
        ...
        <InspectorPanel
          ...
          onDeleteSong={() => {
            if (selectedSong) songWorkspace.deleteSong(selectedSong);
          }}
          onDuplicateSong={() => {
            if (selectedSong) songWorkspace.duplicateSong(selectedSong);
          }}
          onMoveSong={(toIndex) => {
            if (selectedSong) songWorkspace.moveSong(selectedSong, toIndex);
          }}
          onUpdateSong={(changes) => {
            if (selectedSong) songWorkspace.updateSong(selectedSong, changes);
          }}
          {...inspectorActions}
        ...
        <TimelinePanel
          ...
          onAddCue={songWorkspace.addCue}
          onDeleteCue={songWorkspace.deleteCue}
          onDuplicateCue={songWorkspace.duplicateCue}
          onMoveCue={songWorkspace.moveCue}
          onAddLayer={addLayer}
          ...
          onUpdateCue={songWorkspace.updateCue}
